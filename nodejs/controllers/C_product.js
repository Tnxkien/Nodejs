...
async (req, res)=>{
    ...
    if(search!='' && search!=undefined) obj_s['name'] = { $regex: '.*' + search + '.*' };
    ...
    .exec((err, data)=>{
        if(err){
            ...
            res.render('index', {
                ...
                array_table: product_table(array_table, use_admin.get_name_module()),
                search,
                pagination,
                main: 'product/main'
            })
...
router.get('/add', (req, res) => {
    ...
    const arr_option = [
        {name: 'Ốp Lưng Iphone', value: 1},
        {name: 'Phụ Kiện Khác', value: 2},
    ];

    const array_form = [
        {element: 'input', type: 'text', name: 'name', class: 'name', id: 'name', value: '', placeholder: 'Vui lòng nhập Tên', event_changeToSlug: true},
        {element: 'input', type: 'text', name: 'slug', class: 'slug', id: 'slug', value: '', placeholder: 'Vui lòng nhập Slug'},
        {element: 'select', arr_option, name: 'id_parent', class: 'id_parent', id: 'id_parent', value: '', placeholder: ''},
        {element: 'input', type: 'number', name: '_price', class: '_price', id: '_price', value: '', placeholder: 'Vui lòng nhập Giá Tiền'},
        {element: 'textarea', name: 'content', class: 'content', id: 'content', value: '', placeholder: 'Vui lòng nhập Nội Dung', rows: 3, ckeditor: true}
    ];
...
router.post('/processForm', function (req, res) {
    // khai báo
    var _name=_slug=_id_parent=_content=error='', _price=0, flag=1;
    // lấy dữ liệu
    _name=req.body.name;
    _slug=req.body.slug;
                    ...
                    .exec((err, dataSlug)=>{
                        if(err){
                            ...
                            if(dataSlug==''){
                                ...
                                res.send({kq:0, error: 'Dữ liệu đã tồn tại, vui lòng nhập Slug khác nhé.'})
                            }
                        }
                    })
                }else{
                    res.send({kq:0, error: 'Dữ liệu đã tồn tại, vui lòng nhập Tên khác nhé.'})
                }
            }
        })
    }else{
...
router.get('/edit/:id', async (req, res) => {
    ...
    const content_value = rows[0].content;

    const array_form = [
        {element: 'input', type: 'text', name: 'name', class: 'name', id: 'name', value: name_value, placeholder: 'Vui lòng nhập Tên', event_changeToSlug: true},
        {element: 'input', type: 'text', name: 'slug', class: 'slug', id: 'slug', value: slug_value, placeholder: 'Vui lòng nhập Slug'},
        {element: 'select', arr_option, name: 'id_parent', class: 'id_parent', id: 'id_parent', value: '', placeholder: ''},
        {element: 'input', type: 'number', name: '_price', class: '_price', id: '_price', value: price_value, placeholder: 'Vui lòng nhập Giá Tiền'},
        {element: 'textarea', name: 'content', class: 'content', id: 'content', value: content_value, placeholder: 'Vui lòng nhập Nội Dung', rows: 3, ckeditor: true}
    ];
        ...
        .exec((err, data)=>{
            if(err){
                ...
                if(data==''){
                    ...
                    productModel
                    .findByIdAndDelete({_id: req.body.id}, (err)=>{
                        if(err){
                            res.send({kq:0, msg: 'Kết nối Database thất bại.'});
                        }else{
    ...
    array.forEach(e=>{ i++;
        ...
        str+=`<tr class="even pointer" id="id_delete_`+e.id+`">
            ...
            <td class="a-center multil">
                <input type="checkbox" class="flat" name="table_records" value="`+e.id+`">
            </td>
            ...
            <td class=" "><input type="checkbox" id="status_`+e.id+`" `+checked+` onclick="js_status('`+e.id+`')"></td>
            
            <td class=" last">
                <a href="admin/`+link+`/edit/`+e.id+`" class="text-success">
                    <i class="fa fa-edit"></i> Sửa
                </a>
                |
                <a href="javascript:void(0)" class="text-danger" data-toggle="modal"
                    data-target=".bs-example-modal-sm" onclick="get_info_delete('`+e.name+`', '`+e.id+`')">
